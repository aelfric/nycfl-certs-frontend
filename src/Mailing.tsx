...
function Summary(){
    ...
    return <><StripedTable>
        ...
        <tbody>
        {medalCount.map( (row: any) => <tr key={row.schoolId}>
            <td>
                {row.schoolId}
            </td>
            <td>
...
function Detail(){
    ...
    React.useEffect(() => {
        getData("/certs/tournaments/13/awards_sheet", keycloak.token).then(setResults);
    ...
    function doDraft(){
        postData(`/certs/tournaments/13/awards_sheet`, keycloak.token).then(alert).catch(alert);
    }
    ...
    return <><StripedTable>
        ...
        {results.map( row => {
            ...
            return <tr key={row.id}>
                ...
                <td>
                    {contacts.map(email => {
                        if(email.isPrimary){
                            return <p key={email.email}><b>{email.email}</b></p>;
                        } else {
        ...
        <button type="button" className={styles.button} onClick={doDraft}>Draft Emails</button>
        </>;
}
